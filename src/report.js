...
function renderCompactHeader(counts, duration, context = {}) {
  ...
  const columns = [
    { label: 'Passed', value: `${octicon('check-circle-fill-16.svg', 'Passed', context)}<br>**${counts.passed}**` },
    { label: 'Failed', value: `${octicon('x-circle-fill-16.svg', 'Failed', context)}<br>**${counts.failed}**` }
  ];
  if (counts.flaky > 0) {
    columns.push({ label: 'Flaky', value: `${octicon('alert-fill-16.svg', 'Flaky', context)}<br>**${counts.flaky}**` });
  }
  if (counts.other > 0) columns.push({ label: 'Other', value: `•<br>**${counts.other}**` });
...
function renderSummary(data, context = {}) {
  ...
  markdown += '---\n\n';
  markdown += `<sub>Generated by the **Test Automation Reporting action**. [**Tesults**](${TESULTS_URL}) — enhanced test automation reporting and failure intelligence.</sub>\n`;
